...
final class Auto_Post_Plan_Service {
	...
	public function readable_plan_content( Auto_Post_Plan $plan ): string { $outline = $plan->get_outline(); $outline_summary = array(); foreach ( $outline as $item ) { if ( is_array( $item ) ) { $outline_summary[] = isset( $item['heading'] ) ? (string) $item['heading'] : wp_json_encode( $item ); } elseif ( is_scalar( $item ) ) { $outline_summary[] = (string) $item; } } $out = "Auto Post Plan\n\nSummary: " . $plan->summary() . "\nProposed post type: " . $plan->get_proposed_post_type() . "\nDefault status: " . $plan->get_default_status() . "\n\nTitle options:\n- " . implode( "\n- ", $plan->get_title_options() ) . "\n\nOutline:\n- " . implode( "\n- ", array_filter( $outline_summary ) ) . "\n\nSafety notes:\n- " . implode( "\n- ", $plan->get_safety_notes() ) . "\n\nApproval question: " . $plan->get_approval_question(); return $out; }
