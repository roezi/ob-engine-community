...
function obe_full_smoke_find_string_in_php_files( string $base_dir, string $needle ): array {
	$matches = array();
...
$wp_insert_post_matches = obe_full_smoke_find_string_in_php_files(
	ABSPATH . 'includes',
	'wp_insert_post'
);

$allowed_wp_insert_post_path = 'includes/Writing/WordPress_Draft_Writer.php';

obe_full_smoke_assert(
	in_array( $allowed_wp_insert_post_path, $wp_insert_post_matches, true ),
	'Controlled writer path contains wp_insert_post'
);

$unexpected_wp_insert_post_matches = array_values(
	array_diff(
		$wp_insert_post_matches,
		array( $allowed_wp_insert_post_path )
	)
);

obe_full_smoke_assert(
	empty( $unexpected_wp_insert_post_matches ),
	'wp_insert_post appears only in controlled writer path. Unexpected: ' . implode( ', ', $unexpected_wp_insert_post_matches )
);
